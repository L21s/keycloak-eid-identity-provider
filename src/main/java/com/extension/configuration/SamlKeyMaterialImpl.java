...
public class SamlKeyMaterialImpl implements SamlKeyMaterial {
    ...
    SamlKeyMaterialImpl(String samlRequestSignaturePrivateKey, String samlResponseDecryptionPublicKey, String samlResponseDecryptionPrivateKey, String samlResponseVerificationCertificate, String samlRequestEncryptionCertificate) {
    ...
    public KeyPair getSamlResponseDecryptionKeyPair() {
        try {
            ...
            PemReader privateKeyReader = new PemReader(new StringReader(samlResponseDecryptionPrivateKey));
            byte[] privateKeyContent = privateKeyReader.readPemObject().getContent();
            PKCS8EncodedKeySpec privateKeySpec = new PKCS8EncodedKeySpec(privateKeyContent);
    ...
    public X509Certificate getSamlResponseSignatureValidatingCertificate() {
        try {
            ...
            Optional<X509Certificate> cert = Optional.of((X509Certificate) cf.generateCertificate(new ByteArrayInputStream(content)));
