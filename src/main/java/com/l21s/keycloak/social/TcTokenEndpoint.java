...
public class TcTokenEndpoint implements RealmResourceProvider {
  ...
  public Response eIdClientEntrance(@Context UriInfo uriInfo) {
    try {
      logger.info(
          "Received a request on tc-token endpoint. Try to generate a SAML request and redirect to ID PANSTAR.");
      ...
      String authSessionId =
          "_"
              + getQueryParameterOfKey(
                  uriInfo,
                  "authSessionId"); // has to start with _ because xml ids can't start with numbers
      ...
      logger.debug(
          "SAML configuration is {}",
          session.getContext().getRealm().getIdentityProviderByAlias("eid").getConfig().toString());

      String completeUrlWithRequest =
          new SamlRequestGenerator(
