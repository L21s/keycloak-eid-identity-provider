...
public class SamlServiceProviderConfigurationImpl implements SamlServiceProviderConfiguration {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SamlServiceProviderConfigurationImpl.class);
    private final String responseReceiverRealm;
    private final String samlEntityBaseUrl;

    public SamlServiceProviderConfigurationImpl(String responseReceiverRealm, String samlEntityBaseUrl) {
    ...
    public Optional<URL> getSamlResponseReceiverUrl() {
        try {
            URL responseReceiverUrl = new URL(samlEntityBaseUrl + String.format("/realms/%s/broker/eid/endpoint", responseReceiverRealm));
