  ...
  void createsSamlRequestAndRedirectToIdPanstarServer() {
    ...
    try {
      modelConfigMap.put(
          ...
          new String(
              Files.readAllBytes(
                  Paths.get(workingDir + "/keys/01samlRequestSignaturePrivateKey.txt"))));
      modelConfigMap.put(
          "samlResponseDecryptionPublicKey",
          new String(
              Files.readAllBytes(
                  Paths.get(workingDir + "/keys/02samlResponseDecryptionPublicKey.txt"))));
      modelConfigMap.put(
          "samlResponseDecryptionPrivateKey",
          new String(
              Files.readAllBytes(
                  Paths.get(workingDir + "/keys/03samlResponseDecryptionPrivateKey.txt"))));
      modelConfigMap.put(
          "samlResponseVerificationCertificate",
          new String(
              Files.readAllBytes(
                  Paths.get(workingDir + "/keys/04samlResponseVerificationCertificate.txt"))));
      modelConfigMap.put(
          "samlRequestEncryptionCertificate",
          new String(
              Files.readAllBytes(
                  Paths.get(workingDir + "/keys/05samlRequestEncryptionCertificate.txt"))));
    } catch (IOException e) {
    ...
    assertTrue(
        response
            ...
            .getFirst("Location")
            .toString()
            .contains("https://dev.id.governikus-eid.de/gov_autent/async?SAMLRequest="));
  }
}
